...
- **`docker pass`**, a Docker CLI plugin that manages secrets (`set`, `get`, `ls`, `rm`, `run`)
- A **dockerd plugin** that injects resolved secrets into containers at creation
...
- No `docker-auth` or 1Password plugin - only support for [docker-credential-helpers](https://github.com/docker/docker-credential-helpers)
